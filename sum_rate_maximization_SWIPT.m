...
Nt = 6; %transmit antenna number
K = 2;% user number
H = rand(Nt,K)+1i*rand(Nt,K);% wireless channel from BS to users
he =  rand(Nt,1)+1i*rand(Nt,1);%ED channel
iter_num = 100;% iteration number
W = zeros(Nt,K,iter_num+1); % beamfroming matrix

val = zeros(1,iter_num+1);
mu = zeros(K,iter_num+1);
v = zeros(K,iter_num+1);
Pmax = 2;% transmit power budget
sigma = 1;% noise power
delta = 1e-4; % convergence threshold
% initialization
W(:,:,1) = rand(Nt,K)+1i*rand(Nt,K);
mu(:,1) = rand(K,1);
v(:,1) = rand(K,1);
eta = 0.8; % energy transform efficiency
Emax = 8; % target energy threshold
for iter = 1:iter_num
...
figure
plot(val(2:iter+1),'b-o','linewidth',1.5);
